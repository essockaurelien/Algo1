...
//printf("echo che put : %d %d\n", keys1[i], values[i]);
    }
//printf("put andata bene !!!!!\n");     
    /* Search */
    for (i = 0; i < n; ++i)
    {
        int *value = NULL;
...
//printf("value: %ls\n",value);      
        assert( value != NULL );
//printf("assert 1 andata quindi anche le gets !!!!!!\n");
...
//printf("sono found: %d\n", found);
        assert( found != 0 );
//printf("contains eseguita cerca errore al trove. \n");        
    }
    /* Update */
    for (i = 0; i < n; ++i)
    {
    ...
    /* Search */
//printf("io sono qua Update/Search   !!!!\n");
    for (i = 0; i < n; ++i)
    {
        int *value = NULL;
...
void test_traversal()
{
    ...
    visit_context.count = 0;
    upo_bst_traverse_in_order(tree, visitor, &visit_context);
//printf("mi sono fermato qua; colpa della traverse.\n");
    assert( visit_context.count == n);
//printf("Qui non ci arrivo.\n");
